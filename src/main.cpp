...
struct Statistics {
	 ...
	 * Guesses of shannon
	 */
	Guess<double> shannon_entropy;

	/**
	 * Guess of min entropy (currently unused)
	 */
	Guess<double> min_entropy;
...
int count_deterministic(CommandLineArguments &cli, PICounter &counter) {
	...
	vector<vector<string>> results = { { "Input Space Size", atos(SI) }, {
			"Shannon Entropy H(h|l)", atos(shannon_e) }, { "Min Entropy", atos(
			min_e) }, { "Leakage", atos(leakage) }, };
...
int det_shuffle(const CommandLineArguments &cli, PICounter &counter) {
	...
	for (uint i = 0; i < cli.limit(); i++) {
		...
		vector<vector<string>> results = {
				...
				{ "Min Entropy", atos(min_e) }, { "Leakage", atos(leakage) },
		};
		...
		if (cli.statistics()) {
			...
			statistics.shannon_entropy.lower_bound = shannon_entropy(SI,
					lowerbound);
			statistics.shannon_entropy.upper_bound = shannon_e
					+ shannon_entropy_max(SI,
							(SI - statistics.number_of_inputs), SO);
...
int det_iter(const CommandLineArguments &cli, PICounter &counter) {
	...
	for (int k = 1; k <= cli.limit() && b; k++) {
		...
		vector<vector<string>> results = { { "Input Space Size", atos(SI) }, {
				"Shannon Entropy H(h|l)", atos(shannon_e) }, { "Min Entropy",
				atos(min_e) }, { "Leakage", atos(leakage) }, };
