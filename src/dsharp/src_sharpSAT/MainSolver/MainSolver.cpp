...
uint64_t CMainSolver::solve(const char *lpstrFileName) {
    SOLVER_StateT exSt;

#ifdef GMP_BIGNUM
    mpz_class models = mpz_class(0);
    mpz_class one = mpz_class(1);
#else
    unsigned long long int models = 0;
    ...
    if (needToCount) {
        ...
        toSTDOUT("Minisat calls: " << forgottenCompsSAT << " SAT, " <<
                 forgottenCompsUNSAT << " UNSAT, (" <<
                 forgottenCompsKnownSAT << " known SAT, " <<
                 forgottenCompsKnownUNSAT << " known UNSAT) in total\n");
    }
    else {
    ...
    if (rememberedVarNums.size() > 0) {
        DTNode *topNode = decStack.top().getDTNode();
        if (topNode->isBottom()) {
            models = 0;
        }
        else if (topNode->isTop()) {
            models = one << rememberedVarNums.size();
        }
        else {
            set<vector<int> > s = topNode->enumerate();
            for (set<vector<int> >::iterator it = s.begin(); it != s.end(); it++) {
                assert((*it).size() <= rememberedVarNums.size() &&
                       "Encountered a model with more literals than to be remembered");
                models += one << (rememberedVarNums.size() - (*it).size());
            }
        }
        toSTDOUT("Models counted after projection: " << models << "\n");
    }
    ...
    if (CSolverConf::smoothNNF) {
        ...
        if (rememberedVarNums.size() > 0) {
            target_size = rememberedVarNums.size();
            target_vars = rememberedVarNums;
        }
        else {
            target_size = originalVarCount;
            set<int>::iterator it = target_vars.begin();
            for (int i = 1; i <= originalVarCount; ++i) it = target_vars.insert(it, i);
        }

        // See if we've got every literal in the d-DNNF
        if (2 * target_size == literals.size())
            return 0; // WEIGL

        // TODO: Fix an AND node to the top
        if (DT_AND != decStack.top().getDTNode()->getType()) toSTDOUT("Error: The top node wasn't an AND node.");


        // Make sure that every literal appears some place
        DTNode *botNode = new DTNode(DT_AND, num_Nodes++);
        botNode->botIfy();
        for (set<int>::iterator it = target_vars.begin(); it != target_vars.end(); it++) {
...
VarChoiceType CMainSolver::findVSADSDecVar(LiteralIdT &theLit,
                                           const CComponentId &superComp) {
    vector<VarIdT>::const_iterator it;

    int scoreForgotten = -1;
    int scoreRemembered = -1;
    int bo;
    int boRem;
    PVARV pVVForgotten = NULL;
    PVARV pVVRemembered = NULL;
    PVARV pVV = NULL;
    VarChoiceType res;
    ...
    for (it = superComp.varsBegin(); *it != varsSENTINEL; it++) {
        if (getVar(*it).isActive()) {
            ...
            if (getVar(*it).isRemembered()) {
                boRem = (int) getVar(*it).getVSIDSScore() + getVar(*it).getDLCSScore();
                if (boRem > scoreRemembered) {
                    scoreRemembered = boRem;
                    pVVRemembered = &getVar(*it);
                }
            }
            else {
                bo = (int) getVar(*it).getVSIDSScore() + getVar(*it).getDLCSScore();
                if (bo > scoreForgotten) {
                    scoreForgotten = bo;
                    pVVForgotten = &getVar(*it);
                }
            }
        }
    }

    if (pVVRemembered != NULL) {
        pVV = pVVRemembered;
        res = REMEMBERED;
    }
    else if (pVVForgotten != NULL) {
        pVV = pVVForgotten;
        res = FORGOTTEN;
    }
    else
...
bool CMainSolver::decide() {
...
#ifdef SPEAK
    cout << "Entering decide w/" << decStack.TOS_countRemComps() << " comps\n";
#endif
...
#ifdef SPEAK
        cout << "recordRemainingComps()="<<decStack.TOS_countRemComps()<<"\n";
#endif
    ...
    VarChoiceType res = findVSADSDecVar(theLit, decStack.TOS_NextComp());
    if (res == NONE
        || decStack.TOS_NextComp().getClauseCount() == 0) // i.e. component satisfied
    {
#ifdef SPEAK
        toSTDOUT("Going up ("<<decStack.TOS_NextComp().getClauseCount()<<" clauses rem)\n");
#endif
        handleSolution();
    ...
    if (CSolverConf::allowComponentCaching && xFormulaCache.extract(
        ...
        decStack.TOS_popRemComp();
#ifdef SPEAK
        cout << "Cache hit w/ " << cacheVal << " models\n";
#endif
        return false;
    }
...
#ifdef SPEAK
    CComponentId &nxtComp = decStack.TOS_NextComp();
    toSTDOUT("No more remembered vars, "<<nxtComp.getClauseCount()<<" clauses, " << nxtComp.isRemembered() << "\n");
    //printComponent(nxtComp);
    //toSTDOUT("\n");
#endif

        if (((forgottenCompsSAT+forgottenCompsUNSAT+forgottenCompsKnownSAT+forgottenCompsKnownUNSAT) & 0xfff) == 0)
        {
#ifndef SPEAK
            toSTDOUT("Minisat calls: "<<forgottenCompsSAT<<" SAT, "<<forgottenCompsUNSAT<<" UNSAT\n");
#endif
        }

        Minisat::vec<Minisat::Lit> decisions;
        vector<CDecision>::iterator it = decStack.begin();
        it++;
#ifdef SPEAK
                cout << "Assuming ";
#endif SPEAK
        for (; it != decStack.end(); it++)
            {
            int l = (*it).getDTNode()->choiceVar;
            if (l>=0) l = untranslateVar(l);
            else l = -1 * untranslateVar(-1 * l);
            if ((*it).isFlipped()) l = -l;
            if (l>=0) decisions.push(Minisat::mkLit(l, false));
                else decisions.push(Minisat::mkLit(-l, true));
#ifdef SPEAK
                        cout << l << " ";
#endif SPEAK
        }
        ...
        bool kas = decStack.top().isKnownAsSat();
        bool kau = decStack.top().isKnownAsUnsat();

        if (kas)
        {
#ifdef SPEAK
            cout << "*** Component known as SAT\n";
#endif
            forgottenCompsKnownSAT++;
            handleSolution();
            decStack.TOS_popRemComp();
            return false;
        }
        ...
        {
#ifdef SPEAK
            cout << "*** Component known as UNSAT\n";
#endif
            forgottenCompsKnownUNSAT++;
            assert(!kas);
            decStack.TOS_popRemComp();
//			return false;
        }
        else if (solver->solve(decisions))
        {
#ifdef SPEAK
            toSTDOUT("Minisat says SAT\n\n");
#endif
            forgottenCompsSAT++;
            decStack.top().setKnownAsSat(true);
            handleSolution();
            decStack.TOS_popRemComp();
            return false;
        }
        ...
        {
#ifdef SPEAK
            toSTDOUT("Minisat says UNSAT\n\n");
#endif
            forgottenCompsUNSAT++;
            assert(!kas);
            decStack.top().setKnownAsUnsat(true);
            decStack.TOS_popRemComp();
//			return false;
        }
    }
    ...
    //	toSTDOUT("\n");
        ...
        if (theLit.polarity())
        {
            decLit = untranslateVar(theLit.toVarIdx());
        }
        else
        {
            decLit = (-1 * untranslateVar(theLit.toVarIdx()));
        }

        toSTDOUT("\n@"<<decStack.getDL() << "-> " << decLit << "\n");
...
bool CMainSolver::getComp(const VarIdT &theVar, const CComponentId &superComp,
    ...
    for (vt = superComp.varsBegin(); *vt != varsSENTINEL; vt++)
        ...
        {
            CComponentId &c = decStack.lastComp();
            c.addVar(*vt);
            int v = untranslateVar(*vt);
            if (rememberedVarNums.find(v) != rememberedVarNums.end()) c.markRemembered();
