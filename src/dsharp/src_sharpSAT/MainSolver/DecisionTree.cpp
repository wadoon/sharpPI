...
void DTNode::compressNode()
{
	...
	case DT_TOP:
		break;
	case DT_LIT:
		if (rememberedVarNums.empty()) break;
		if (var < 0) var = - var;
		if (rememberedVarNums.find(var) == rememberedVarNums.end())
		{
			type = DT_TOP;
		}
		break;
...
set<vector<int> > DTNode::enumerate()
{
	if (DT_LIT == type) 
	{
		vector<int> v;
	...
	{
		set<DTNode *>::iterator it = children.begin();
		set<vector<int> > s = (*it)->enumerate();
		it++;
		for (; it != children.end(); it++)
		{
			set<vector<int> > from = (*it)->enumerate();
			set<vector<int> > to;
			set<vector<int> >::iterator jt;
			for (jt = s.begin(); jt != s.end(); jt++)
			{
				vector<int> prefix = (*jt);
				set<vector<int> >::iterator kt;
				for (kt = from.begin(); kt != from.end(); kt++)
				{
					vector<int> suffix = (*kt);
					vector<int> s1 = prefix;
					s1.insert(s1.end(), suffix.begin(), suffix.end());
					to.insert(s1);
				}
			}
			s = to;
	...
	else if (DT_OR == type)
	{
		...
		{
			set<vector<int> > from = (*it)->enumerate();
			s.insert(from.begin(),from.end());
		}
		return s;

	} else assert(0 && "Found top/bottom node.");
}
