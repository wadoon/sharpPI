...
public:
	...
	int untranslateVar(int val)
	{
		return origTranslation[varTranslation[val]];
	}
...
protected:
	...
	static void print_translation(const string s, const vector<int> trans)
	{
		toSTDOUT("Translation:" << endl);
		for (int i = 0; i < trans.size(); ++i)
		{
			if (i!=trans[i]) toSTDOUT(s << ":" << i << " -> " << trans[i] << endl);
		}
	}
