...
bool CInstanceGraph::createfromFile(const char* lpstrFileName)
{
...
#ifdef WITH_MINISAT
	for (int i=0; i<=nVars; i++) solver->newVar();
	...
	while (inFile.getline(buf, BUF_SZ))
	{
		...
		if (buf[0] == 'c')
				...
				while (buf[i] != 0x0)
				{
					...
					while (buf[i] != 0x0 && (buf[i] < '0' || buf[i]	> '9'))
						i++;
...
#ifdef WITH_MINISAT
				Minisat::Lit a = (lit > 0) ?
					Minisat::mkLit(lit, false) :
					Minisat::mkLit(-lit, true);
				clause_1.push(a);
#endif
			}
		}
	...
	cout << "Projection scope:";
	for (set<int>::iterator it = rememberedVarNums.begin(); it != rememberedVarNums.end(); it++)
	{
		cout << " " << (*it);
	}
	cout << "\n";
	...
	if (!rememberedVarNums.empty())
        {
		DepositOfVars::iterator vit;
		for (vit = beginOfVars(); vit != endOfVars(); vit++)
		{
			if (rememberedVarNums.find((*vit).getVarNum()) == rememberedVarNums.end()) (*vit).forget();
		}
        }
