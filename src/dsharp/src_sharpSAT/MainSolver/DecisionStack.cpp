...
bool CDecisionStack::flipTOS()
{
    ...
    top().setKnownAsSat(false);
    top().setKnownAsUnsat(false);
...
bool CDecisionStack::pop()
{
    ...
    bool kas = back().isKnownAsSat();

    pop_back();

    if (kas) back().setKnownAsSat(true);
...
{
    bool kau = back().isKnownAsUnsat();
    push_back(CDecision(other));
    if (kau) back().setKnownAsUnsat(true);
...
void CDecisionStack::TOS_sortRemComps()
{
    ...
    for (int i = uLower; i <=uUpper;i++) cout << keys[i]->isRemembered();
    cout << "->";
#endif
    ...
    for (int i = uLower; i <=uUpper;i++)
        ...
        {
            // process remembered components first
            if ((keys[i]->isRemembered() > keys[j]->isRemembered()) ||
                (keys[i]->isRemembered() == keys[j]->isRemembered() &&
                 keys[i]->countVars() < keys[j]->countVars()))
            {
                vBuf = keys[i];
                keys[i] = keys[j];
                keys[j] = vBuf;
